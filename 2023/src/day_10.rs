...
pub fn part_1(contents: &str) -> Result<u64, String> {
    ...
    traverse(&grid, START, row, col).ok_or("No path found".to_string())
}
...
fn traverse(grid: &[&str], char: char, row: usize, col: usize) -> Option<u64> {
    // If the current pipe can connect to the north,
    // see if the pipe above can also connect to the south.
    if char == START || char == NORTH_SOUTH || char == NORTH_WEST || char == NORTH_EAST {
        if let Some(line) = grid.get(row - 1) {
            if let Some(char) = line.chars().nth(col) {
                ...
                if char == NORTH_SOUTH || char == SOUTH_WEST || char == SOUTH_EAST {
                    if let Some(steps) = traverse(grid, char, row - 1, col) {
                        return Some(steps + 1);
                    }
                }
            }
    ...
    if char == START || char == NORTH_SOUTH || char == SOUTH_WEST || char == SOUTH_EAST {
        if let Some(line) = grid.get(row + 1) {
            if let Some(char) = line.chars().nth(col) {
                if char == START {
                    return Some(0);
    ...
    if char == START || char == EAST_WEST || char == NORTH_WEST || char == SOUTH_WEST {
        if let Some(char) = grid[row].chars().nth(col - 1) {
            ...
            if char == EAST_WEST || char == NORTH_EAST || char == SOUTH_EAST {
                if let Some(steps) = traverse(grid, char, row, col - 1) {
                    return Some(steps + 1);
                }
            }
        }
    ...
    if char == START || char == EAST_WEST || char == NORTH_EAST || char == SOUTH_EAST {
        if let Some(char) = grid[row].chars().nth(col + 1) {
            if char == START {
                return Some(0);
            }
