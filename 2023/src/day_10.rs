...
fn traverse(grid: &[&str], char: char, from: Direction, row: usize, col: usize) -> Option<u64> {
    ...
    if from != Direction::North && (char == NORTH_SOUTH || char == NORTH_WEST || char == NORTH_EAST)
    {
        if let Some(line) = grid.get(row - 1) {
            if let Some(char) = line.chars().nth(col) {
                if char == START {
    ...
    if from != Direction::South && (char == NORTH_SOUTH || char == SOUTH_WEST || char == SOUTH_EAST)
    {
        if let Some(line) = grid.get(row + 1) {
            if let Some(char) = line.chars().nth(col) {
                if char == START {
    ...
    if from != Direction::West && (char == EAST_WEST || char == NORTH_WEST || char == SOUTH_WEST) {
        if let Some(char) = grid[row].chars().nth(col - 1) {
            if char == START {
                return Some(0);
            }
    ...
    if from != Direction::East && (char == EAST_WEST || char == NORTH_EAST || char == SOUTH_EAST) {
        if let Some(char) = grid[row].chars().nth(col + 1) {
            if char == START {
                return Some(0);
            }
    ...
    panic!("Pipe `{}` at {}, {} cannot connect to anything coming from direction {:?}", char, row, col, from);
}
